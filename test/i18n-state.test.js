...
// --- audit ---------------------------------------------------------------
{
  ...
  const audit = S.auditLanguage({ enDict: en, ruDict: ru, langDict: de, state });
  ...
  const after = S.auditLanguage({ enDict: en2, ruDict: ru, langDict: de, state });
  ...
  const withNew = S.auditLanguage({ enDict: en3, ruDict: ru, langDict: de, state });
...
ok('buildState: records when the language was verified', (() => {
  ...
  return st.verifiedAt === '2026-07-25T10:00:00Z';
})());
ok('buildState: baselining then auditing leaves nothing to do', (() => {
  ...
  const a = S.auditLanguage({ enDict: en, ruDict: ru, langDict: uk, state: st });
...
ok('audit: accepts a bare {key: hash} map as well as a full state file', (() => {
  ...
  const bare = S.auditLanguage({ enDict: en, ruDict: ru, langDict: uk, state: full.keys });
