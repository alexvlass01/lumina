...
ok('buildGelbooruTagSuggestUrl: official DAPI tag endpoint', parsedUrl.origin + parsedUrl.pathname === T.GELBOORU_TAG_API);
ok('buildGelbooruTagSuggestUrl: uses anonymous tag pattern', parsedUrl.searchParams.get('name_pattern') === 'blue_hai%' && !parsedUrl.searchParams.has('user_id') && !parsedUrl.searchParams.has('api_key'));
